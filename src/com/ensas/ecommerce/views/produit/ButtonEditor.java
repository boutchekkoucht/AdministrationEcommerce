...
public class ButtonEditor extends DefaultCellEditor{
	...
	public Object getCellEditorValue() {
		 ...
		 if(clicked){
			 ...
			 Produit pro= m.getPro().getProduit(Long.parseLong(lbl));
			 
			 if(action==1)
			 {
				new DeleteProduit(pro,index); 
			 }
			 else if(action==0)
			 {
				 new UpdateProduit(pro, index);
			 }
			 else
			 {
			 index.getBody().removeAll();
			 			 
			 index.getBody().add(new com.ensas.ecommerce.views.produit.Produit(pro));
